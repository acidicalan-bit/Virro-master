...
set search_path = pg_catalog, public
as $$
  select encode(digest(convert_to(public.build002_canonical_json(p_value), 'UTF8'), 'sha256'), 'hex')
$$;
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('requirementId', requirement_id, 'signalId', signal_id::text, 'contentHash', content_hash) order by requirement_id, signal_id::text, content_hash), '[]'::jsonb) into v_current_refs from public.build002_signals where owner_tenant_id = v_tenant and outcome_transaction_id = v_tx.id;
...
comment on function public.build002_grant_execution_authority(uuid, uuid, uuid, uuid, text) is 'BUILD002-C1-D4-R2: UTF-8 canonical hash authority and current signal content_hash binding; preserves R1 authority invariants.';
