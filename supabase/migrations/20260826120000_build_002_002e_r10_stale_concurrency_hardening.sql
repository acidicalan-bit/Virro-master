...
begin
  ...
  -- Discover missing canonical lineage before the complete fence set is built.
  if v_transaction is not null and p_operation <> 'direct.outcome_transactions.insert' then
    ...
    if found then
      v_tenant := coalesce(v_tenant, v_tx.owner_tenant_id);
      v_asset := coalesce(v_asset, v_tx.asset_id);
      v_version := coalesce(v_version, v_tx.base_version_id);
    end if;
  end if;
  ...
  if v_asset is not null and v_tenant is null then
  ...
  if v_membership is not null then
    ...
    if found then
      v_tenant := coalesce(v_tenant, v_membership_row.tenant_id);
      v_principal := coalesce(v_principal, v_membership_row.principal_id);
    end if;
  end if;
  ...
  if v_membership is not null then
    ...
    if not found or v_membership_row.tenant_id is distinct from v_tenant
       or (v_principal is not null and v_membership_row.principal_id is distinct from v_principal) then
      raise exception 'BUILD002_002E_IDENTITY_CHANGED_RESTART_REQUIRED' using errcode = '40001';
    end if;
  end if;
...
begin
  ...
  if nullif(current_setting('build002.udre_active_operation', true), '') is not null then
    return new;
  end if;
...
as $$
begin
  if nullif(current_setting('build002.udre_active_operation', true), '') is null then
...
begin
  ...
  perform public.build002_002e_route(p_operation, p_classification, p_context);
  perform set_config('build002.udre_active_operation', p_operation, true);
  return true;
end;
$$;
...
begin
  ...
  v_entered := public.build002_002e_enter('rpc.revoke_tenant_membership','MATERIAL_WRITER',jsonb_build_object(
    'tenant_id',v_target.tenant_id,'principal_id',p_actor_principal_id,'member_principal_id',v_target.principal_id));
