...
## BUILD 005-B provider geometry closure

The previous real smoke used a 1×1 PNG. The old adapter mapped that source through `resolveSize()` to `256x256`; OpenAI rejected it before returning an image because the current `gpt-image-2` contract requires a materially larger pixel budget. The adapter now keeps this provider-specific capability profile behind `OpenAIImageEditExecutor`: dimensions must be positive safe integers, divisible by 16, within the current edge/pixel limits, and within a 3:1 aspect ratio. Precision Edit uses same-geometry execution: source, requested output, RAW, preserved candidates, and final deliverable retain the same canvas; it never silently resizes, crops, pads, or uses `auto`.

Invalid source geometry returns `SOURCE_GEOMETRY_UNSUPPORTED_BY_CURRENT_PROVIDER` before any provider request. A provider response with unexpected dimensions returns `PROVIDER_OUTPUT_CONTRACT_VIOLATION` and is not admitted to preservation. The deterministic final smoke fixture is 1024×1024 PNG artwork with a structured checkerboard background and a central orange marker; the 1×1 fixture remains a negative preflight regression only. Provider geometry is recorded in adapter metadata, while customer Task Spec semantics remain provider-neutral.

The installed OpenAI SDK (`7.4.0`) documents `gpt-image-2` arbitrary `WIDTHxHEIGHT` sizes with 16-pixel divisibility, 1:3–3:1 aspect ratio, maximum supported resolution 3840×2160, and standard 1024×1024/1536×1024/1024×1536 sizes. This gate does not claim permanent provider capability; future SDK/API drift requires a new capability review.
