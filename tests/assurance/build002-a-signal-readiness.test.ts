...
describe("BUILD 002-A E1 signal qualification and readiness matrix", () => {
  ...
  it("does not delegate an expired READY assessment", () => { const r = requirement(); const s = signal(r, "x"); const q = qualify(r, [s]); const a = ready(r, q, dependency([s]), { validUntil: "2026-08-18T11:59:00.000Z" }); expect(evaluateReadinessValidity(a, dependency([s]), at)).toBe("EXPIRED"); expect(isDelegable(a, "EXPIRED")).toBe(false); });
