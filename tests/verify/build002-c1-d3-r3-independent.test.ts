...
const PRODUCT_SHA = "3c1481fb7f7bdbe243cbfe2a272d32358eded879";
const PRODUCT_TREE = "5faf576de38640f11165fdaa02ea7daf1c2e2e53";
const VERIFIED_R3_SHA = "40a954a88612e0af04fc6cdafd102d594d9163a4";
...
const predecessorTests = [
  "tests/native/build002-c0-c-postgres.e3.test.ts",
  "tests/native/build002-c0-e-authority-chain.e3.test.ts",
  "tests/native/build002-c1-a-postgres.e3.test.ts",
...
function predecessorGlobalMigrationRatchet(source: string): boolean {
  return /expect\(migrations\)\.toHaveLength\(\s*\d+\s*\)/.test(source)
    || /expect\(readdirSync\([\s\S]*?supabase\/migrations[\s\S]*?\)\)\.toHaveLength\(\s*\d+\s*\)/.test(source)
    || /migrations(?:\.sort\(\))?\.at\(-1\)/.test(source);
}

function readPredecessors(): Map<string, string> {
  return new Map(predecessorTests.map((path) => [
    path,
...
describe("BUILD002-C1-D3-R3 independent verifier", () => {
  it("pins product ancestry, tree and forward-only migration history", () => {
    ...
    expect(git("merge-base", "--is-ancestor", VERIFIED_R3_SHA, PRODUCT_SHA)).toBe("");
  ...
  it("proves predecessor tests do not own global migration cardinality or latest-name ratchets", () => {
    const sources = readPredecessors();
    for (const source of sources.values()) expect(predecessorGlobalMigrationRatchet(source)).toBe(false);
  });

  it("rejects reintroduced predecessor migration count 33 or 34", () => {
    const source = sourcesForAttack().get("tests/native/build002-c0-c-postgres.e3.test.ts")!;
    const count33 = source.replace(
    ...
    const count34 = count33.replace("toHaveLength(33)", "toHaveLength(34)");
    expect(predecessorGlobalMigrationRatchet(count33)).toBe(true);
    expect(predecessorGlobalMigrationRatchet(count34)).toBe(true);
  });

  it("rejects reintroduced predecessor latest-migration assertions", () => {
    const source = sourcesForAttack().get("tests/native/build002-c0-e-authority-chain.e3.test.ts")!;
    const latest = `${source}\nexpect(migrations.at(-1)).toBe(\"20260823110000_build_002_c1_d3_r3_retry_admissibility.sql\");\n`;
    expect(predecessorGlobalMigrationRatchet(latest)).toBe(true);
  });
  ...
  function sourcesForAttack(): Map<string, string> {
    return readPredecessors();
  }
});
