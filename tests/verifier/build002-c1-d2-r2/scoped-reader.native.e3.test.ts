...
const databaseUrl = process.env.BUILD002_VERIFIER_NATIVE_PG_URL ?? process.env.BUILD002_NATIVE_PG_C1_D2_URL;
...
class PgSupabaseLikeClient {
  ...
  from(table: string) {
    expect(table).toBe("build002_readiness_authority_commits");
    return {
...
describe.runIf(enabled && Boolean(databaseUrl))("D2-R2 independent native PostgreSQL 17 scoped reader", () => {
  let admin: Client;
  let isolatedDatabase = "";

  beforeAll(async () => {
    isolatedDatabase = `virro_d2_r2_${process.pid}_${Date.now()}`;
    ...
    await admin.query("insert into public.projects(id, name, owner_tenant_id) values ($1, 'D2 R2 A', $2), ($3, 'D2 R2 B', $4)", ["70000000-0000-4000-8000-000000000001", TENANT_A, "70000000-0000-4000-8000-000000000002", TENANT_B]);
    await admin.query("insert into public.assets(id, project_id, name, owner_tenant_id) values ($1, $2, 'D2 R2 A', $3), ($4, $5, 'D2 R2 B', $6)", ["71000000-0000-4000-8000-000000000001", "70000000-0000-4000-8000-000000000001", TENANT_A, "71000000-0000-4000-8000-000000000002", "70000000-0000-4000-8000-000000000002", TENANT_B]);
  ...
  it("proves PostgreSQL 17 scoped query hides a foreign marker ID", async () => {
