...
const enabled = process.env.BUILD002_V1_NATIVE === "true";
const mode = process.env.BUILD002_V1_MODE === "r1" ? "r1" : "r2";
const databaseUrl = process.env.BUILD002_V1_DATABASE_URL ?? process.env.BUILD002_NATIVE_PG_URL;
const migrationsDir = resolve(process.env.BUILD002_V1_MIGRATIONS_DIR ?? resolve(process.cwd(), "supabase/migrations"));
...
describe.runIf(enabled && Boolean(databaseUrl))(`BUILD002-C1-D5-R2-V1 ${mode.toUpperCase()} independent evidence`, () => {
