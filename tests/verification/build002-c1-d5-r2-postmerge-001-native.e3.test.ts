...
const enabled = process.env.BUILD002_POSTMERGE_NATIVE === "true";
const mode = process.env.BUILD002_POSTMERGE_MODE === "r1" ? "r1" : "r2";
const databaseUrl = process.env.BUILD002_POSTMERGE_DATABASE_URL ?? process.env.BUILD002_NATIVE_PG_URL;
const migrationsDir = resolve(process.env.BUILD002_POSTMERGE_MIGRATIONS_DIR ?? resolve(process.cwd(), "supabase/migrations"));
...
describe.runIf(enabled && Boolean(databaseUrl))(`BUILD002-C1-D5-R2-POSTMERGE-001 ${mode.toUpperCase()} exact-merge evidence`, () => {
  ...
  beforeAll(async () => {
    isolatedDatabase = `virro_d5_r2_v1_${mode}_${process.pid}_${Date.now()}`;
