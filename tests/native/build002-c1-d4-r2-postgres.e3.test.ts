...
describe.runIf(enabled && Boolean(databaseUrl))("BUILD002-C1-D4-R2 native TaskSpec authority closure", () => {
  ...
  beforeAll(async () => {
    ...
    const migrations = readdirSync(migrationsDir).filter((name) => name.endsWith(".sql")).sort(); expect(migrations).toHaveLength(38); for (const name of migrations) await admin.query(readFileSync(resolve(migrationsDir, name), "utf8"));
