...
describe.runIf(enabled && Boolean(databaseUrl))("BUILD002-C1-D3-R3 native PostgreSQL 17 evidence closure", () => {
  ...
  beforeAll(async () => {
    ...
    const migrations = readdirSync(migrationsDir).filter((name) => name.endsWith(".sql")).sort(); expect(migrations).toContain("20260823110000_build_002_c1_d3_r3_retry_admissibility.sql"); for (const name of migrations) await admin.query(readFileSync(resolve(migrationsDir, name), "utf8"));
    value = graph();
  ...
  async function waitUntilBlocked(blockedPid: number, blockerPid: number, timeoutMs = 10_000): Promise<void> { const deadline = Date.now() + timeoutMs; await new Promise((resolve) => setTimeout(resolve, 50)); while (Date.now() < deadline) { const row = await admin.query("select $1 = any(pg_blocking_pids($2)) as blocked", [blockerPid, blockedPid]); if (row.rows[0].blocked) return; await new Promise((resolve) => setTimeout(resolve, 50)); } const activity = await admin.query("select pid, state, wait_event_type, wait_event, pg_blocking_pids(pid) as blocking_pids, left(query, 160) as query from pg_stat_activity where pid = any($1::int[]) order by pid", [[blockedPid, blockerPid]]); const locks = await admin.query("select pid, locktype, relation::regclass as relation, mode, granted from pg_locks where pid = any($1::int[]) and relation is not null order by pid, relation::regclass::text, mode", [[blockedPid, blockerPid]]); throw new Error(`D3_CONCURRENCY_TIMEOUT blocked=${blockedPid} blocker=${blockerPid} activity=${JSON.stringify(activity.rows)} locks=${JSON.stringify(locks.rows)}`); }
  ...
  it("serializes Signal insertion in both orders", async () => {
    ...
    const firstA = await pid(serviceA); const firstB = await pid(serviceB); const blockedAdmission = admit(serviceA); await waitUntilBlocked(firstA, firstB); await serviceB.query("commit"); await expect(blockedAdmission).rejects.toThrow(/CURRENTNESS_NOT_CURRENT|SERIALIZED_RECHECK_FAILED/); await cleanupExtras();
    const admission = await admitInTransaction(serviceA); await serviceB.query("begin"); const secondA = await pid(serviceA); const secondB = await pid(serviceB); const blockedWriter = insertRequirement(serviceB).then(() => insertSignal(serviceB)); await waitUntilBlocked(secondB, secondA); await serviceA.query("commit"); await blockedWriter; await serviceB.query("commit"); await cleanupExtras();
    expect(admission.admission_id).toEqual(expect.any(String));
  }, 60_000);
  it("serializes requirement insertion in both orders", async () => {
    await serviceB.query("begin"); await insertRequirement(serviceB, secondRequirement(), REQUIREMENT3); const firstA = await pid(serviceA); const firstB = await pid(serviceB); const blockedAdmission = admit(serviceA); await waitUntilBlocked(firstA, firstB); await serviceB.query("commit"); await expect(blockedAdmission).rejects.toThrow(/CURRENTNESS_NOT_CURRENT|SERIALIZED_RECHECK_FAILED/); await cleanupExtras();
    const admission = await admitInTransaction(serviceA); await serviceB.query("begin"); const secondA = await pid(serviceA); const secondB = await pid(serviceB); const blockedWriter = insertRequirement(serviceB, secondRequirement(), REQUIREMENT3); await waitUntilBlocked(secondB, secondA); await serviceA.query("commit"); await blockedWriter; await serviceB.query("commit");
    expect(admission.admission_id).toEqual(expect.any(String));
  }, 60_000);
  it("serializes source-head changes in both orders", async () => {
    ...
    await serviceB.query("begin"); await serviceB.query("update public.assets set current_version_id=$1 where id=$2", [VERSION2, ASSET]); const firstA = await pid(serviceA); const firstB = await pid(serviceB); const blockedAdmission = admit(serviceA); await waitUntilBlocked(firstA, firstB); await serviceB.query("commit"); await expect(blockedAdmission).rejects.toThrow(/SOURCE_ASSET_HEAD_CHANGED|CURRENTNESS_NOT_CURRENT/); await cleanupExtras();
    await admin.query("insert into public.asset_versions(id, asset_id, version_number, state, owner_tenant_id) values ($1,$2,2,'{\"width\":101}'::jsonb,$3) on conflict do nothing", [VERSION2, ASSET, TENANT]); const admission = await admitInTransaction(serviceA); await serviceB.query("begin"); const secondA = await pid(serviceA); const secondB = await pid(serviceB); const blockedWriter = serviceB.query("update public.assets set current_version_id=$1 where id=$2", [VERSION2, ASSET]); await waitUntilBlocked(secondB, secondA); await serviceA.query("commit"); await blockedWriter; await serviceB.query("commit"); await cleanupExtras();
    expect(admission.admission_id).toEqual(expect.any(String));
  }, 60_000);
  it("serializes membership revocation in both orders", async () => {
    await cleanupExtras();
    await serviceB.query("begin"); await serviceB.query("update public.tenant_memberships set status='REVOKED' where id=$1", [MEMBERSHIP]); const firstA = await pid(serviceA); const firstB = await pid(serviceB); const blockedAdmission = admit(serviceA); await waitUntilBlocked(firstA, firstB); await serviceB.query("commit"); await expect(blockedAdmission).rejects.toThrow(/READINESS_AUTHORITY_MEMBERSHIP_INVALID|AUTHORITY_NOT_CURRENT/); await cleanupExtras();
    const admission = await admitInTransaction(serviceA); await serviceB.query("begin"); const secondA = await pid(serviceA); const secondB = await pid(serviceB); const blockedWriter = serviceB.query("update public.tenant_memberships set status='REVOKED' where id=$1", [MEMBERSHIP]); await waitUntilBlocked(secondB, secondA); await serviceA.query("commit"); await blockedWriter; await serviceB.query("commit"); await cleanupExtras();
    expect(admission.admission_id).toEqual(expect.any(String));
  }, 60_000);
  it("serializes transaction raw_request changes in both orders", async () => {
    ...
    await serviceB.query("begin");
    const secondA = await pid(serviceA); const secondB = await pid(serviceB); const blockedWriter = serviceB.query("update public.outcome_transactions set raw_request='d3 changed' where id=$1", [TRANSACTION]);
    await waitUntilBlocked(secondB, secondA);
