...
describe.runIf(enabled && Boolean(databaseUrl))("BUILD002-C1-D3-R3 native PostgreSQL 17 evidence closure", () => {
  ...
  it("serializes source-head changes in both orders", async () => {
    ...
    const admission = await admitInTransaction(serviceA); await serviceB.query("begin"); const secondA = await pid(serviceA); const secondB = await pid(serviceB); const blockedWriter = serviceB.query("update public.assets set current_version_id=$1 where id=$2", [VERSION2, ASSET]); await waitUntilBlocked(secondB, secondA); await serviceA.query("commit"); await blockedWriter; await serviceB.query("commit"); await cleanupExtras();
    expect(admission.admission_id).toEqual(expect.any(String));
  }, 60_000);
