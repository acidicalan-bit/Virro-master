...
describe.runIf(enabled && Boolean(databaseUrl))("BUILD002-C1-D3-R2 native PostgreSQL 17 evidence closure", () => {
  ...
  beforeAll(async () => {
    ...
    await admin.query("set application_name='d3-r2-admin'"); await serviceA.query("set application_name='d3-r2-authority'; set statement_timeout='5s'; set lock_timeout='4s'"); await serviceB.query("set application_name='d3-r2-mutator'; set statement_timeout='5s'; set lock_timeout='4s'");
    ...
    await anon.query("set role anon; set application_name='d3-r2-anon'"); await authenticated.query("set role authenticated; set application_name='d3-r2-authenticated'");
    const migrations = readdirSync(migrationsDir).filter((name) => name.endsWith(".sql")).sort(); expect(migrations).toHaveLength(33); for (const name of migrations) await admin.query(readFileSync(resolve(migrationsDir, name), "utf8"));
    value = graph();
  ...
  it("rejects forged current material, non-ready states, expired readiness, evaluator drift, and scope mismatches", async () => {
    ...
    for (const readinessState of ["READY_WITH_CONDITIONS", "NEEDS_CONTEXT", "INSUFFICIENT_SIGNAL", "HUMAN_REVIEW_REQUIRED", "BLOCKED_BY_POLICY"]) {
      const forged = structuredClone(request()) as unknown as Record<string, unknown>;
      forged.readinessState = readinessState;
      await expect(call(forged)).rejects.toThrow();
    }
    const expired = structuredClone(request()) as unknown as Record<string, unknown>;
    expired.revalidatedAt = new Date(Date.now() - 60_000).toISOString();
    await expect(call(expired)).rejects.toThrow();
  ...
  it("serializes requirement insertion in both orders", async () => {
    await cleanupExtras();
    await serviceB.query("begin"); await insertRequirement(serviceB); const blockedAdmission = admit(serviceA); await waitUntilBlocked(serviceA, serviceB); await serviceB.query("commit"); await expect(blockedAdmission).rejects.toThrow(/CURRENTNESS_NOT_CURRENT|SERIALIZED_RECHECK_FAILED/); await cleanupExtras();
    const admission = await admitInTransaction(serviceA); await serviceB.query("begin"); const blockedWriter = insertRequirement(serviceB); await waitUntilBlocked(serviceB, serviceA); await serviceA.query("commit"); await blockedWriter; await serviceB.query("commit"); await cleanupExtras();
    expect(admission.admission_id).toEqual(expect.any(String));
  });
  ...
  it("serializes transaction raw_request changes in both orders", async () => {
    await cleanupExtras();
    await serviceB.query("begin"); await serviceB.query("update public.outcome_transactions set raw_request='d3 changed' where id=$1", [TRANSACTION]); const blockedAdmission = admit(serviceA); await waitUntilBlocked(serviceA, serviceB); await serviceB.query("commit"); await expect(blockedAdmission).rejects.toThrow(/CURRENTNESS_NOT_CURRENT|SERIALIZED_RECHECK_FAILED/); await cleanupExtras();
    const admission = await admitInTransaction(serviceA); await serviceB.query("begin"); const blockedWriter = serviceB.query("update public.outcome_transactions set raw_request='d3 changed' where id=$1", [TRANSACTION]); await waitUntilBlocked(serviceB, serviceA); await serviceA.query("commit"); await blockedWriter; await serviceB.query("commit"); await cleanupExtras();
    expect(admission.admission_id).toEqual(expect.any(String));
  });
});
