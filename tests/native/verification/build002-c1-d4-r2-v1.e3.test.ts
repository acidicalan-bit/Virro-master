...
const enabled = process.env.BUILD002_V1_NATIVE_PG_C1_D4_R2 === "true";
const databaseUrl = process.env.BUILD002_V1_NATIVE_PG_C1_D4_R2_URL ?? process.env.BUILD002_V1_NATIVE_PG_URL;
const migrationsDir = resolve(process.cwd(), "supabase/migrations");
const ACTOR = "a2100000-0000-4000-8000-000000000001";
const ACTOR_B = "a2100000-0000-4000-8000-000000000002";
const TENANT = "b2100000-0000-4000-8000-000000000001";
const MEMBERSHIP = "b2100000-0000-4000-8000-000000000002";
const MEMBERSHIP_B = "b2100000-0000-4000-8000-000000000003";
const PROJECT = "c2100000-0000-4000-8000-000000000001";
const ASSET = "d2100000-0000-4000-8000-000000000001";
const VERSION = "e2100000-0000-4000-8000-000000000001";
const TRANSACTION = "f2100000-0000-4000-8000-000000000001";
const RUN = "a3100000-0000-4000-8000-000000000001";
const CANDIDATE = "a4100000-0000-4000-8000-000000000001";
const OUTCOME = "a5100000-0000-4000-8000-000000000001";
const BLUEPRINT = "a6100000-0000-4000-8000-000000000001";
const PROFILE = "a6100000-0000-4000-8000-000000000002";
const SIGNAL = "a7100000-0000-4000-8000-000000000001";
const SIGNAL_DRIFT = "a7100000-0000-4000-8000-000000000002";
const REQUIREMENT = "a8100000-0000-4000-8000-000000000001";
const REQUIREMENT_DRIFT = "a8100000-0000-4000-8000-000000000002";
const QUALIFICATION = "a9100000-0000-4000-8000-000000000001";
const READINESS = "ab000000-0000-4000-8000-000000000001";
...
describe.runIf(enabled && Boolean(databaseUrl))("BUILD002-C1-D4-R2-V1 independent authority verification", () => {
  ...
  beforeAll(async () => {
    isolatedDatabase = `virro_d4_v1_${process.pid}_${Date.now()}`;
    ...
    const migrations = readdirSync(migrationsDir).filter((name) => name.endsWith(".sql")).sort(); expect(migrations).toHaveLength(37); for (const name of migrations) await admin.query(readFileSync(resolve(migrationsDir, name), "utf8"));
  ...
  it("compares the production TypeScript canonical contract with PostgreSQL", async () => {
    const generatedComposites = Array.from({ length: 5 }, (_, index) => ({
      schemaVersion: "v1-composite",
      index,
      values: [index, -index, index / 10, { z: `z-${index}`, a: [true, null, `u-${index}`] }],
      nested: { keys: { b: index + 1, a: index - 1 } },
    }));
    const fixtures: unknown[] = [null, true, false, "", "ASCII", "quotes\"", "backslash\\", "line\nfeed", "tab\t", "Unicode á", "emoji 😀", "e\u0301", [], ["x"], [[1], [true, null]], [true, "x", 1], {}, { z: 1, a: 2 }, { nested: { array: [{ b: 2, a: 1 }] } }, 0, -0, 1, -1, 1.5, 0.1, 0.01, 0.000001, 0.0000001, 1e-7, 1e20, 1e21, 9007199254740991, -9007199254740991, ...generatedComposites];
  ...
  it("rejects currentness and identity drift before authority issuance", async () => {
    ...
    const versionB = "e2100000-0000-4000-8000-000000000002";
