...
import { ArrowRight, BookOpenCheck, Boxes, Camera, CheckCircle2, FileOutput, MapPinned, Radar, ShieldCheck } from "lucide-react";
import { useLanguage } from "@/components/i18n/language-provider";
import { StaggerGroup } from "@/components/landing/motion/motion-primitives";

const capabilities = [
  { icon: Camera, name: "Understanding Capture", en: "Captures real operating information from meetings, screens, tickets, documents, calls, designs and systems.", es: "Captura información operativa real desde juntas, pantallas, tickets, documentos, calls, diseños y sistemas." },
  { icon: Radar, name: "Readiness Gate", en: "Checks whether information can move forward or still needs context, criteria, evidence or a decision.", es: "Valida si la información puede avanzar o todavía necesita contexto, criterio, evidencia o una decisión." },
  { icon: Boxes, name: "Understanding Event", en: "Creates a traceable unit around what must be understood, by whom and for which action.", es: "Crea una unidad trazable sobre qué debe entenderse, por quién y para qué acción." },
  { icon: FileOutput, name: "Output Bundles", en: "Packages receiver-specific questions, checklists, context and next actions.", es: "Empaqueta preguntas, checklists, contexto y siguientes acciones específicos para cada receptor." },
  { icon: MapPinned, name: "Living Understanding Map", en: "Keeps flows, decisions, risks, tools, handoffs and understanding debt connected over time.", es: "Mantiene conectados flujos, decisiones, riesgos, herramientas, handoffs y deuda de entendimiento." },
  { icon: BookOpenCheck, name: "Knowledge Continuity", en: "Makes visible where operating context can disappear when people, teams, vendors or tools change.", es: "Hace visible dónde puede desaparecer el contexto operativo cuando cambian personas, equipos, proveedores o herramientas." },
  { icon: ShieldCheck, name: "Privacy Shield", en: "Applies minimization, masking and pattern-first handling from audit to pilot.", es: "Aplica minimización, máscara y manejo pattern-first desde la auditoría hasta el piloto." },
];

export function PlatformCapabilities() {
  const { locale, t } = useLanguage();
  return <section id="capacidades" className="capability-system-section scroll-mt-24 px-5 py-24 md:px-8 md:py-36"><div className="mx-auto max-w-[1380px]"><div className="grid gap-8 lg:grid-cols-[.78fr_1.22fr] lg:items-end"><div><p className="section-kicker">{t("The operating system", "El sistema operativo")}</p><h2 className="section-display">{t("From captured information to a living understanding layer.", "De información capturada a una capa viva de entendimiento.")}</h2></div><p className="section-lead lg:justify-self-end">{t("Virro connects evidence, readiness and receiver-specific deliverables so understanding does not disappear after one analysis.", "Virro conecta evidencia, readiness y entregables por receptor para que el entendimiento no desaparezca después de un análisis.")}</p></div><div className="capability-system-rail mt-14" aria-label={t("Virro capability sequence", "Secuencia de capacidades Virro")}><span>Capture</span><ArrowRight size={14} /><span>Understanding Event</span><ArrowRight size={14} /><span>Readiness Gate</span><ArrowRight size={14} /><span>Output Bundle</span><ArrowRight size={14} /><span>Living Map</span><ArrowRight size={14} /><span>Executive Report</span></div><StaggerGroup className="capability-system-grid mt-5">{capabilities.map(({ icon: Icon, name, en, es }, index) => <article key={name} className={index === 4 ? "is-featured" : ""}><div><span>{String(index + 1).padStart(2, "0")}</span><Icon size={19} /></div><h3>{name}</h3><p>{locale === "es" ? es : en}</p><CheckCircle2 size={14} /></article>)}</StaggerGroup></div></section>;
}
