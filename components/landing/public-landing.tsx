...
export function PublicLanding() {
  return <div className="min-h-screen overflow-hidden bg-[var(--app-bg)] text-[var(--text)]">
    ...
    <main>
      ...
      <section className="border-y border-white/[.055] bg-white/[.018] px-5 py-12 md:px-8 md:py-14"><div className="mx-auto grid max-w-7xl gap-7 lg:grid-cols-[.72fr_1.28fr] lg:items-center"><div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-teal-300">Entregable del diagnóstico</p><h2 className="mt-3 text-2xl font-semibold tracking-[-.04em] md:text-3xl">Qué recibes en un diagnóstico</h2><p className="mt-3 max-w-xl text-xs leading-6 text-[var(--muted)]">Evidencia ejecutiva para decidir qué corregir primero, sin evaluar personas ni producir más documentación innecesaria.</p></div><div className="grid gap-2 sm:grid-cols-2">{deliverables.map((item) => <div key={item} className="flex items-start gap-2 rounded-lg border border-white/[.07] bg-[var(--panel)] px-3 py-2.5 text-[10px] leading-4 text-[var(--muted)]"><CheckCircle2 size={12} className="mt-0.5 shrink-0 text-teal-300" />{item}</div>)}</div></div></section>
      ...
      <section className="px-5 py-20 text-center md:px-8 md:py-24"><div className="mx-auto max-w-3xl"><h2 className="text-3xl font-semibold tracking-[-.045em] md:text-5xl">Evalúa un flujo donde tu empresa está perdiendo entendimiento.</h2><p className="mx-auto mt-5 max-w-2xl text-sm leading-6 text-[var(--muted)]">Empieza por un dolor real. Virro ayuda a convertirlo en evidencia, preguntas críticas y una primera acción operativa.</p><p className="mx-auto mt-3 max-w-2xl text-[10px] leading-5 text-[var(--subtle)]">No necesitas saber qué pack elegir. Empieza describiendo el flujo donde hoy se pierde claridad.</p><a href="#audit" className="mt-7 inline-flex h-12 items-center gap-2 rounded-xl bg-teal-300 px-6 text-sm font-semibold text-slate-950">Diagnosticar un flujo crítico <ArrowRight size={15} /></a></div></section>
    </main>
