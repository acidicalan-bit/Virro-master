...
export function PublicLanding() {
  ...
  return <div className="public-landing min-h-screen overflow-hidden bg-[var(--app-bg)] text-[var(--text)]">
    ...
    <main id="main-content">
      ...
      <section id="producto" className="product-stage scroll-mt-24 px-5 py-24 md:px-8 md:py-36"><div className="mx-auto max-w-[1380px]"><div className="grid gap-7 lg:grid-cols-[.82fr_1.18fr] lg:items-end"><div><p className="section-kicker">{t("Product interface", "Interfaz de producto")}</p><h2 className="section-display">{t("From scattered information to operational signals for better decisions.", "De información dispersa a señales operativas para decidir mejor.")}</h2></div><p className="section-lead lg:justify-self-end">{t("Virro retains readiness, risk, context and continuity signals that inform decisions, handoffs and executive reports.", "Virro conserva señales de readiness, riesgo, contexto y continuidad que alimentan decisiones, handoffs y reportes ejecutivos.")}</p></div><div className="product-module-rail" aria-label={t("Core product modules", "Módulos centrales del producto")}>{["Capture", "Privacy Shield", "Readiness Gate", "Pack Engine", "Executive Report"].map((module, index) => <span key={module}><i>{String(index + 1).padStart(2, "0")}</i>{module}</span>)}</div><ProductWalkthrough /></div></section>
