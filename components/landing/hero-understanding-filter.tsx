  ...
  return <section id="plataforma" className="landing-hero relative min-h-[820px] scroll-mt-24 overflow-hidden px-5 pb-24 pt-36 md:px-8 md:pt-44">
    ...
    <div className="relative mx-auto grid max-w-[1380px] gap-14 xl:grid-cols-[.88fr_1.12fr] xl:items-center">
      <div className="hero-copy-scene relative z-10">
        ...
        <h1 className="hero-scene-item mt-7 max-w-4xl text-[2.8rem] font-semibold leading-[.97] tracking-[-.065em] sm:text-6xl lg:text-[4.6rem]">{t("Reduce the cost of working with misunderstood information.", "Reduce el costo de trabajar con información mal entendida.")}</h1>
        <p className="hero-scene-item mt-7 max-w-2xl text-base font-medium leading-7 text-[var(--text)] md:text-lg">{t("Virro reviews whether information moving across teams, processes, tools and AI has enough context, criteria and clarity to move forward with lower risk.", "Virro revisa si la información que se mueve entre áreas, procesos, herramientas e IA tiene suficiente contexto, criterio y claridad para avanzar con menor riesgo.")}</p>
        <p className="hero-scene-item mt-4 max-w-2xl text-xs leading-6 text-[var(--muted)] md:text-sm">{t("It makes missing context, receiver expectations and the next recommended action visible before information becomes work or a decision.", "Hace visible el contexto faltante, lo que necesita el receptor y la siguiente acción recomendada antes de que la información se convierta en trabajo o decisión.")}</p>
      ...
      <RevealOnScroll className="hero-visual-reveal"><div className="hero-filter-scene hero-product-scene" aria-label={t("Information flowing through Virro's understanding filter", "Información atravesando el filtro de entendimiento de Virro")}>
        ...
        <div className="filter-scene-body">
          <div className="filter-inputs"><p>{t("Ambiguous information", "Información ambigua")}</p>{inputLabels.map((label, index) => <span key={label} style={{ "--particle-index": index } as React.CSSProperties}>{label}</span>)}</div>
          <div className="filter-core"><div className="filter-core-rings" aria-hidden /><div className="filter-core-mark"><Radar size={20} /><strong>Virro Core</strong><small>Privacy-first engine</small></div><div className="filter-signals">{filters.map((label, index) => <span key={label} style={{ "--signal-index": index } as React.CSSProperties}>{locale === "es" ? label : ["Context", "Criteria", "Receiver", "Readiness"][index]}</span>)}</div></div>
