...
import { AUDIT_CONSENT_VERSION, AUDIT_LEAD_RETENTION_DAYS, auditFlowOptions, auditToolOptions } from "@/lib/conversion/audit-request";
...
export function DiagnosisRequestForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setStatus(result.code === "DELIVERY_UNAVAILABLE" ? "fallback" : "error");
        setError(result.error || t("The request could not be sent.", "No se pudo enviar la solicitud."));
        trackPublicEvent(result.code === "RATE_LIMITED" ? "audit_form_rate_limited" : "audit_form_failed");
        return;
      }
      setStatus("success"); form.reset(); trackPublicEvent("audit_form_submitted");
    } catch (reason) { setStatus("fallback"); setError(reason instanceof Error ? reason.message : t("The request could not be sent.", "No se pudo enviar la solicitud.")); trackPublicEvent("audit_form_failed"); }
  }

  if (status === "success") return <div id="solicitar-auditoria" className="scroll-mt-24 rounded-2xl border border-teal-400/20 bg-teal-400/[.06] p-7" role="status"><CheckCircle /><h3 className="mt-4 text-2xl font-semibold">{t("Request received.", "Solicitud recibida.")}</h3><p className="mt-3 text-sm leading-6 text-[var(--muted)]">{t("We will review the described flow and respond within two business days.", "Revisaremos el flujo descrito y responderemos en un máximo de dos días hábiles.")}</p><Link href="/app/demo-scenarios" className="brand-secondary-button mt-6">{t("Explore the enterprise demo", "Explorar demo enterprise")} <ArrowRight aria-hidden="true" size={14} /></Link></div>;
  ...
  return <form id="solicitar-auditoria" onSubmit={submit} onFocusCapture={markStarted} aria-describedby="audit-sensitive-note" className="diagnosis-request-form scroll-mt-24 rounded-2xl border border-white/[.08] bg-[var(--panel)] p-5 shadow-[0_24px_80px_rgba(0,0,0,.2)] md:p-7">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <FormField id="audit-flow" label={t("Workflow type", "Tipo de flujo")}><select id="audit-flow" name="flowType" required defaultValue="" className="field-control"><option value="" disabled>{t("Select a workflow", "Selecciona un flujo")}</option>{auditFlowOptions.map(option => <option key={option}>{option}</option>)}</select></FormField>
      ...
      <FormField id="audit-tool" label={t("Primary tool", "Herramienta principal")}><select id="audit-tool" name="tool" required defaultValue="" className="field-control"><option value="" disabled>{t("Select a tool", "Selecciona una herramienta")}</option>{auditToolOptions.map(option => <option key={option}>{option}</option>)}</select></FormField>
    ...
    <input type="hidden" name="consentVersion" value={AUDIT_CONSENT_VERSION} />
    ...
    {status === "fallback" && <div role="alert" className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/[.06] p-4 text-[11px] leading-5 text-amber-100"><p className="font-semibold">{t("The secure channel is temporarily unavailable.", "El canal seguro no está disponible temporalmente.")}</p><p className="mt-1">{error} {t("Your form remains on this device so you can retry without sending it through another channel.", "Tu formulario permanece en este dispositivo para que puedas reintentar sin enviarlo por otro canal.")}</p></div>}
  </form>;
}
