    ...
    <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
      ...
      <form onSubmit={submit} className="rounded-2xl border border-white/[.08] bg-[var(--panel)] p-5 shadow-[0_24px_80px_rgba(0,0,0,.2)] md:p-7">
        <div className="grid gap-4 sm:grid-cols-2">
          <FormField label="Nombre"><input name="name" required autoComplete="name" className="field-control" /></FormField>
          <FormField label="Empresa"><input name="company" required autoComplete="organization" className="field-control" /></FormField>
          <FormField label="Rol"><input name="role" required autoComplete="organization-title" className="field-control" /></FormField>
          <FormField label="Email"><input name="email" type="email" required autoComplete="email" className="field-control" /></FormField>
          ...
          <FormField label="Describe brevemente dónde se está perdiendo claridad, contexto o ejecución." className="sm:col-span-2"><textarea name="context" required minLength={12} className="field-control min-h-32 resize-y py-3 leading-6" /></FormField>
        </div>
