...
export function PrivacyTrust() {
  ...
  const current = mode === "private" ? {
    ...
    description: t("Store permissioned, anonymized understanding patterns for workspace-level detection.", "Guarda patrones anonimizados y autorizados para detección a nivel workspace."),
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 xl:grid-cols-[.88fr_1.12fr]">
      ...
      <div className="space-y-4"><article className="panel p-5"><h2 className="text-sm font-semibold">{t("Workspace data mode", "Modo de datos del workspace")}</h2><p className="mt-1 text-[10px] text-[var(--subtle)]">{t("Choose what Virro may retain after an Understanding Event is analyzed.", "Elige qué puede retener Virro después de analizar un Understanding Event.")}</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><ModeButton active={mode === "private"} onClick={() => setMode("private")} icon={<EyeOff size={17} />} title={t("Private Mode", "Modo privado")} detail={t("Minimal retention by default", "Retención mínima por defecto")} /><ModeButton active={mode === "pattern"} onClick={() => setMode("pattern")} icon={<Fingerprint size={17} />} title={t("Pattern Learning Allowed", "Aprendizaje por patrones permitido")} detail={t("Anonymized, permissioned patterns", "Patrones anonimizados y autorizados")} /></div><div className="mt-4 rounded-xl border border-teal-400/15 bg-teal-400/[.045] p-4"><div className="flex items-center gap-2"><span className="size-2 rounded-full bg-teal-300" /><p className="text-xs font-semibold">{current.label} {t("active", "activo")}</p></div><p className="mt-2 text-[10px] leading-5 text-[var(--muted)]">{current.description}</p></div></article>
    ...
    <section className="panel p-5"><div className="grid gap-4 md:grid-cols-4">{[
      { n: "01", title: t("Capture", "Capturar"), detail: t("Only relevant operational input", "Solo input operativo relevante") },
