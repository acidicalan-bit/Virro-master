...
export function InboxWorkbench() {
  ...
  return <div className="space-y-6">
    <section>
      ...
      <p className="mt-2 max-w-3xl text-[11px] leading-5 text-[var(--subtle)]">{t("The assistant does not converse for the sake of conversation. It turns ambiguous information into an auditable Understanding Event.", "El asistente no conversa por conversar. Convierte información ambigua en un Understanding Event auditable.")}</p>
    </section>
...
function EmptyState({ t }: { t: (english: string, spanish: string) => string }) { const evidence = [t("Recommended pack", "Pack recomendado"), t("Meaning Loss risks", "Riesgos de Meaning Loss"), t("Missing context", "Contexto faltante"), t("Critical questions", "Preguntas críticas"), t("Estimated scores", "Scores estimados"), t("Audit / pilot recommendation", "Recomendación de audit / piloto")]; return <div className="flex min-h-[700px] flex-col items-center justify-center text-center"><div className="grid size-14 place-items-center rounded-2xl border border-[var(--border)] bg-[var(--panel-soft)] text-[var(--subtle)]"><FileText size={22} /></div><h3 className="mt-4 text-sm font-medium">{t("Analysis result", "Resultado del análisis")}</h3><p className="mt-2 max-w-sm text-xs leading-5 text-[var(--subtle)]">{t("Complete the intake to receive auditable evidence instead of an open-ended conversation.", "Completa el input para recibir evidencia auditable, no una conversación abierta.")}</p><div className="mt-6 grid w-full max-w-lg grid-cols-2 gap-2 text-left text-[9px] text-[var(--subtle)] sm:grid-cols-3">{evidence.map((item) => <span key={item} className="rounded-lg border border-[var(--border)] bg-[var(--panel-soft)] px-3 py-2.5">{item}</span>)}</div></div>; }
